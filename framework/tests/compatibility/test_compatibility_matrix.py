...
""")
    ...
    def test_current_python_version_support(self, quality_gates_action, test_project):
        ...
        assert current_version >= (3, 10), f"Running on Python {current_version}, need >=3.10"
        assert current_version < (3, 13), f"Running on Python {current_version}, tested up to 3.12"
    ...
    def test_python_version_detection_in_config(self, quality_gates_action, test_project):
        ...
        if "dependencies" in config["tool"]["pixi"]:
            ...
            if "python" in deps:
                ...
                assert "3.10" in python_req or "3.11" in python_req or "3.12" in python_req
    
    @pytest.mark.parametrize("python_version_spec", [
        "3.10.*",
        "3.11.*", 
        "3.12.*",
        ">=3.10",
    ...
    def test_python_version_requirement_parsing(self, quality_gates_action, python_version_spec):
...
class TestPlatformCompatibility:
    ...
    def test_current_platform_support(self, quality_gates_action):
        ...
        assert current_platform in ["linux", "darwin", "windows"], f"Unsupported platform: {current_platform}"
...
""")
    ...
    def test_multi_platform_project_detection(self, quality_gates_action, platform_config):
...
class TestPackageManagerCompatibility:
    ...
    @pytest.mark.parametrize("manager_config", [
        {
            "name": "pixi",
    ...
    def test_package_manager_detection_and_commands(self, quality_gates_action, manager_config):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            (project_dir / "pyproject.toml").write_text(manager_config["config_section"])
        ...
        for i, file_content in enumerate(test_cases):
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                if not file_content or "pyproject.toml" not in file_content:
                    ...
                    assert manager.name in ["pip", "pixi"]  # May detect pixi from invalid content
...
class TestDependencyCompatibility:
    ...
    def test_compatibility_matrix_summary(self):
        ...
        python_supported = sum(1 for status in compatibility_matrix["python_versions"].values() if status.startswith("✅"))
        platform_supported = sum(1 for status in compatibility_matrix["platforms"].values() if status.startswith("✅"))
        manager_supported = sum(1 for status in compatibility_matrix["package_managers"].values() if status.startswith("✅"))

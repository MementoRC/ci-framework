...
class TemplateValidator:
    ...
    def __init__(
        self, templates_dir: Path, reference_projects: list[Path] | None = None
    ):
    ...
    def _validate_pyproject_template(
        self, template_path: Path, level: ValidationLevel
    ) -> ValidationResult:
    ...
    def _validate_precommit_template(
        self, template_path: Path, level: ValidationLevel
    ) -> ValidationResult:
    ...
    def _validate_dependabot_template(
        self, template_path: Path, level: ValidationLevel
    ) -> ValidationResult:
    ...
    def _validate_security_template(
        self, template_path: Path, level: ValidationLevel
    ) -> ValidationResult:
    ...
    def _is_compatible_with_project(
        self, template_path: Path, project_path: Path
    ) -> bool:
    ...
    def generate_report(self) -> str:
        ...
        if not self.validation_results:
            return (
                "No validation results available. Run validate_all_templates() first."
            )
        ...
        avg_compatibility = (
            sum(r.compatibility_score for r in self.validation_results)
            / total_templates
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Validate CI Framework configuration templates"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--reference-projects", nargs="*", help="Reference project paths"
    )
    ...
    if args.reference_projects:
        reference_projects = [
            Path(p) for p in args.reference_projects if Path(p).exists()
        ]

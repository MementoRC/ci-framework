...
class QualityGatesAction:
    ...
    def _execute_commands_parallel(self, commands: List[str], project_dir: Path, 
                                 timeout: int) -> Dict[str, Dict[str, Any]]:
    ...
    def _execute_commands_sequential(self, commands: List[str], project_dir: Path,
                                   timeout: int, fail_fast: bool = True) -> Dict[str, Dict[str, Any]]:
        ...
        for cmd in commands:
            ...
            if not result["success"] and fail_fast:
                if "F821" in result.get("stderr", "") or "E9" in result.get("stderr", ""):
                    # Critical lint violation
                    break
        
        return results
    
    def _generate_reports(self, project_dir: Path, results: Dict[str, Dict[str, Any]], 
                         tier: str) -> None:
...
<coverage version="7.0.0">
    ...
    def execute_tier(self, project_dir: Union[str, Path], tier: str, 
                    timeout: Optional[int] = None, parallel: bool = True,
                    config_overrides: Optional[Dict[str, Any]] = None,
                    dry_run: bool = False) -> QualityResult:
        """
        ...
        try:
            # Execute commands
            if parallel:
                command_results = self._execute_commands_parallel(commands, project_dir, timeout)
            else:
                command_results = self._execute_commands_sequential(commands, project_dir, timeout)
            ...
            result.partial_success = len(successful_checks) > 0 and len(failed_checks) > 0
            ...
            result.failure_reason = "timeout"
            result.error_details = f"Overall execution timed out after {timeout} seconds"
        except Exception as e:
            result.success = False

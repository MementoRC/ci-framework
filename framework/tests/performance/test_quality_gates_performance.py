...
""")
            ...
            test_dir.mkdir()
            for i in range(20):
                (test_dir / f"test_{i}.py").write_text(f"def test_function_{i}(): assert True")
            
            yield project_dir
    
    def test_small_project_performance_baseline(self, quality_gates_action, small_mock_project):
    ...
    def test_medium_project_performance_scaling(self, quality_gates_action, medium_mock_project):
        ...
        with patch('subprocess.run') as mock_run:
            ...
            start_time = time.time()
            result = quality_gates_action.execute_tier(
                project_dir=medium_mock_project,
                tier="extended",
                dry_run=True
            )
            ...
            assert execution_time < 2.0, f"Medium project took {execution_time}s, should be <2.0s"
            assert result.success is True
    ...
    def test_parallel_vs_sequential_performance(self, quality_gates_action, medium_mock_project):
    ...
    def test_timeout_performance_handling(self, quality_gates_action, small_mock_project):
        ...
        with patch('subprocess.Popen') as mock_popen:
            ...
            assert timeout_handling_time < 3.0, f"Timeout handling took {timeout_handling_time}s, should be <3.0s"
    ...
    def test_configuration_override_performance(self, quality_gates_action, small_mock_project):
        ...
        with patch('subprocess.run') as mock_run:
            ...
            assert config_processing_time < 1.0, f"Config processing took {config_processing_time}s, should be <1.0s"
            assert result.success is True
    ...
    def test_real_project_performance_benchmark(self, quality_gates_action):
        ...
        projects = [
            ("/home/memento/ClaudeCode/Project/hb-strategy-sandbox/worktrees/feat-workspace-phase2", "large"),
            ("/home/memento/ClaudeCode/Servers/cheap-llm/worktrees/feat-phase1", "medium"),
        ]
        ...
        for project_path, size_category in projects:
            ...
            if size_category == "medium":
                assert pattern_time < 0.5, f"Medium project pattern detection: {pattern_time}s"
                assert dry_run_time < 1.0, f"Medium project dry run: {dry_run_time}s"
            elif size_category == "large":
                assert pattern_time < 2.0, f"Large project pattern detection: {pattern_time}s"
...
""")
            ...
            assert concurrent_time < 5.0, f"Concurrent execution took {concurrent_time}s, should be <5.0s"

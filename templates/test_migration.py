...
class MigrationResult:
    ...
    performance_impact: float | None  # Execution time change
...
class ProjectMigrationTester:
    ...
    def test_all_migrations(
        self, reference_projects: list[Path]
    ...
    def _test_pyproject_migration(self, project_path: Path) -> MigrationResult:
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                migration_status, test_errors = self._test_project_functionality(
                    temp_project
                )
        ...
        if existing_pyproject.exists():
            try:
                ...
                if authors and isinstance(authors[0], dict):
                    values["author_name"] = authors[0].get(
                        "name", values["author_name"]
                    )
                    values["author_email"] = authors[0].get(
    ...
    def _test_project_functionality(
        ...
        try:
            ...
            result = subprocess.run(
                ["pixi", "install"], capture_output=True, text=True, timeout=300
            )
            ...
            if partial_success:
                return (
                    MigrationStatus.PARTIAL if errors else MigrationStatus.SUCCESS,
                    errors,
                )
            else:
                return MigrationStatus.FAILED, errors
    ...
    def _measure_performance_impact(
        self, project_path: Path, original_config: Path | None, new_config: Path
    ) -> float | None:
    ...
    def generate_migration_report(self) -> str:
        ...
        success_tests = sum(
            1 for r in self.test_results if r.status == MigrationStatus.SUCCESS
        )
        partial_tests = sum(
            1 for r in self.test_results if r.status == MigrationStatus.PARTIAL
        )
        failed_tests = sum(
            1 for r in self.test_results if r.status == MigrationStatus.FAILED
        )
        ...
        for project_name, results in projects.items():
            ...
            for result in results:
                ...
                template_type = result.template_name.replace(".toml", "").replace(
                    ".yaml", ""
                )
                report.append(
                    f"- **{template_type}**: {status_emoji} {result.status.value} ({result.quality_score:.1%})"
                )
...
def main():
    ...
    parser.add_argument(
        "--templates-dir", default="./templates", help="Templates directory"
    )
    parser.add_argument(
        "--projects", nargs="+", required=True, help="Reference project paths"
    )

...
class TestHBStrategyIntegration:
    ...
    def test_hb_project_exists(self, hb_project_path):
        ...
        if not hb_project_path.exists():
            pytest.skip("hb-strategy-sandbox project not available for integration testing")
    ...
    def test_essential_tier_command_mapping(self, quality_gates_action, hb_project_path):
    ...
    def test_configuration_compatibility(self, quality_gates_action, hb_project_path):
        ...
        if "environments" in pixi_config:
            ...
            quality_envs = [env for env in env_names if "quality" in env or "dev" in env or "ci" in env]
    ...
    def test_environment_isolation_detection(self, quality_gates_action, hb_project_path):
    ...
    def test_framework_folder_compatibility(self, hb_project_path):
        ...
        if framework_dir.exists():
            ...
            assert len(python_files) > 0, "Framework directory should contain Python files"
            ...
            existing_ci_files = [
                f for f in potential_ci_files 
                if (framework_dir / f).exists() or 
                any((framework_dir / subdir / f).exists() for subdir in ["ci", "quality", "testing"] if (framework_dir / subdir).exists())
            ]
...
class TestHBStrategyDropInReplacement:
    ...
    def test_backup_and_restore_simulation(self, hb_project_path):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert original_files == backup_files, "Backup should contain all original files"
    ...
    def test_establish_baseline_metrics(self, hb_project_path):
        ...
        baseline_metrics = {
            "project_size": {
                ...
                "directories": len([d for d in hb_project_path.rglob("*") if d.is_dir()])
            },
            "configuration_complexity": {
                "pyproject_toml_size": (hb_project_path / "pyproject.toml").stat().st_size if (hb_project_path / "pyproject.toml").exists() else 0,
        ...
        if pyproject_path.exists():
            try:
                ...
                if "tool" in config and "pixi" in config["tool"]:
                    baseline_metrics["configuration_complexity"]["has_pixi_environments"] = True
                    if "environments" in config["tool"]["pixi"]:
                        baseline_metrics["configuration_complexity"]["environment_count"] = len(config["tool"]["pixi"]["environments"])
            except:
                pass

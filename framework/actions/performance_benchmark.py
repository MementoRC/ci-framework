...
class PerformanceBenchmarkAction:
    ...
    def setup_benchmark_environment(self, project_dir: Path, package_manager: str) -> bool:
    ...
    def parse_benchmark_results(self, benchmark_data: Dict[str, Any]) -> List[BenchmarkResult]:
    ...
    def load_baseline_results(self, baseline_dir: Path) -> Optional[List[BenchmarkResult]]:
    ...
    def analyze_regression(
        ...
        for current in current_results:
            ...
            if baseline.mean > 0:
                percentage_change = ((current.mean - baseline.mean) / baseline.mean) * 100
            else:
        ...
        for current in current_results:
            ...
            if len(historical_means) >= 2:
                ...
                if len(historical_means) > 1:
                    recent_trend = (historical_means[-1] - historical_means[-2]) / historical_means[-2] * 100
                else:
                    recent_trend = 0.0
                ...
                numerator = sum((i - x_mean) * (y - y_mean) for i, y in enumerate(historical_means))
                ...
                trends[benchmark_name] = {
                    ...
                    "trend_direction": "improving" if overall_trend < 0 else "degrading" if overall_trend > 0 else "stable"
                }
    ...
    def execute_benchmarks(
        ...
        if compare_baseline:
            ...
            if baseline_results:
                regression_detected, regression_analyses, max_regression = self.analyze_regression(
                ...
                baseline_comparison = f"Compared against {len(baseline_results)} baseline benchmarks"
                if regression_detected:
                    baseline_comparison += f", regression detected: {max_regression:.2f}%"
                else:
